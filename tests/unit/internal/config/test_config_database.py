...
from app.domain.financing.model import Financing
from app.internal.config import (DATABASE_HOST, DATABASE_NAME, DATABASE_PORT,
                                 DATABASE_USER)
...
def test_set_up_database_tables(metadata_create_all):
    ...
    assert f"{DATABASE_SCHEMA}.{Financing.__tablename__}" in table_names
